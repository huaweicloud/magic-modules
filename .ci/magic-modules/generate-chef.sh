...
# It outputs "chef-generated", a folder containing at least one non-submodule git repo containing the
# generated chef code.
...
for PRD in "${PRODUCT_ARRAY[@]}"; do
  pushd magic-modules-branched
    git submodule update --init "build/chef/$PRD"

    LAST_COMMIT_AUTHOR="$(git log --pretty="%an <%ae>" -n1 HEAD)"
    find build/chef/"${PRD}"/ -type f -not -name '.git*' -print0 | xargs -0 rm -rf --
    bundle install
    ...
    bundle exec compiler -p "products/$PRD" -e chef -o "build/chef/$PRD" 2> "/chef-$PRD.log"

    # This command can crash - if that happens, the script should not fail.
    set +e
    CHEF_COMMIT_MSG="$(python .ci/magic-modules/extract_from_pr_description.py --tag "chef-$PRD" < .git/body)"
    set -e
    if [ -z "$CHEF_COMMIT_MSG" ]; then
      CHEF_COMMIT_MSG="Magic Modules changes."
    fi
    ...
    pushd "build/chef/$PRD"
      ...
      git commit -m "$CHEF_COMMIT_MSG" --author="$LAST_COMMIT_AUTHOR" || true  # don't crash if no changes
      git checkout -B "$(cat ../../../branchname)"
    popd
  popd
  git clone "magic-modules-branched/build/chef/$PRD" "chef-generated/$PRD"
